...
# Source tree layout is <workspace>/stonr/app. In desktop bundles, app is under
# <bundle>/Resources/<slug>/app where <slug> is the project root.
PROJECT_ROOT=$(CDPATH= cd -- "$APP_DIR/.." && pwd -P)
WORKSPACE_CANDIDATE=$(CDPATH= cd -- "$PROJECT_ROOT/.." 2>/dev/null && pwd -P || printf '%s\n' "$PROJECT_ROOT")
REPO_ROOT=$PROJECT_ROOT
if [ -f "$WORKSPACE_CANDIDATE/Cargo.toml" ] \
  && grep -q '^\[workspace\]' "$WORKSPACE_CANDIDATE/Cargo.toml" 2>/dev/null; then
  REPO_ROOT=$WORKSPACE_CANDIDATE
fi
...
resolve_stonr_bin() {
  ...
  if [ -x "$PROJECT_ROOT/target/debug/stonr" ]; then
    printf '%s\n' "$PROJECT_ROOT/target/debug/stonr"
    return 0
  fi
...
ensure_stonr_bin() {
  ...
  if ! cargo build --quiet --manifest-path "$PROJECT_ROOT/Cargo.toml"; then
